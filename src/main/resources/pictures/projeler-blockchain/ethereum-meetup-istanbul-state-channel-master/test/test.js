...
contract("StateChannel", (accounts) => {
  ...
  describe('ChannelTimeout', () => {
    let SC
    before(async () => {
      SC = await PaymentChannel.new(receiver, 999, {from: sender, value: 10000000})
    }
    it('should revert if the Timout is still active', async () => {
      const rec = await SC.recipient.call()
      rec.should.be.equal(receiver)
      const senderContVal = await  SC.owner.call()
      senderContVal.should.be.equal(sender)
      const _timeout = SC.timeout.call()
      _timeout.should.be.bignumber.equal(timeout)
    })
